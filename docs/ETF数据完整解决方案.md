...
## 十、增量更新与分时Fallback机制（v2.2 新增）

> **版本**: v2.2 | **日期**: 2026-03-18

### 10.1 增量更新逻辑

**目标**：只请求缺失的交易日，不请求全部历史

**实现位置**：`data_maintenance.py`

**核心函数**：
```python
# 1. 计算缺失的交易日
missing_days = calculate_missing_trading_days(from_date, to_date)
...
**交易日判断**：
- 排除周末（周六、周日）
- 排除节假日（读取 `data/holiday.txt`）
- 交易时间内（9:30-15:00）只到昨天，非交易时间可以到今天

### 10.2 分时数据Fallback机制

**目标**：东财接口不稳定时，用分时数据聚合生成日线

**数据来源**：
1. `data/sector-minute-warmup.json` - warmup缓存的分时数据
2. `data/minute_data/minute_*.jsonl` - 原始分时文件
...
**目标**：东财恢复后，对比分时聚合数据和东财数据，有差异则用东财覆盖

**流程**：
```
1. 尝试请求东财日线数据
   ↓ 失败
2. 用分时数据聚合生成日线（最后一分钟=收盘价）
   ↓
3. 东财恢复后，对比分时聚合日线和东财日线
   ↓
4. 有差异则用东财数据覆盖（确保准确性）
```
