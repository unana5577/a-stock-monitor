...
class Indicators:
    ...
    def calculate_score(
        ...
        复用fetch_sector_data.py的_score_rotation_item逻辑
        ...
        base = momentum_map.get(momentum, 0) + behavior_map.get(fund_behavior, 0)
        score = base + alpha_5 * 0.15 + alpha_20 * 0.05 + amount_share_change * 2.0

        # 操作建议惩罚
        if action:
            if "回避" in action or "离场" in action or "止损" in action:
                score -= 4
            elif "止盈" in action:
                score -= 1
    ...
    def detect_false_kill(
        alpha_20: float,
        amount_share_pct: float,
        market_down_ratio: float = 0,
        market_return: float = 0,
        amount_share_p80: float = None
    ) -> bool:
        """检测错杀机会

        复用sector_lifecycle.py的detect_false_kill逻辑

        条件：
        1. 板块处于上升期（alpha_20 > 0 或 amount_share >= amount_share_p80）
        2. 大盘恐慌（下跌家数占比 > 65% 或 大盘跌幅 > 1%）

        Args:
            alpha_20: 20日Alpha
            amount_share_pct: 当前成交占比（%）
            market_down_ratio: 大盘下跌家数占比
            market_return: 大盘涨跌幅
            amount_share_p80: 成交占比80分位数（可选）
        ...
        if amount_share_p80 is not None:
            sector_up = sector_up or amount_share_pct >= amount_share_p80

        # 判断大盘是否恐慌
        market_panic = market_down_ratio > 0.65 or market_return <= -1

        return sector_up and market_panic
